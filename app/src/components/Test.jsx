...
function reducer(state, action) {
  ...
  sourceList.cards = sourceList.cards.filter((card) => card !== cardContent);
...
case 'DELETE_CARD':
  const { listId: deleteListId, cardContent: deleteCardContent } = action.payload;
...
function App() {
  ...
  const onDrop = (e, listId) => {
    ...
    if (sourceListId !== listId) {
      ...
      dispatch({ type: 'MOVE_CARD', payload: { sourceListId, destListId: listId, cardContent } });
  
      // Remove the card from the source list
      dispatch({ type: 'DELETE_CARD', payload: { listId: sourceListId, cardContent } });
    }
    ...
    <div className="App">
      <div className="board">
        {state.lists.map((list) => (
          <div key={list.id} className="list" onDrop={(e) => onDrop(e, list.id)} onDragOver={(e) => onDragOver(e)}>
            <h2>{list.title}</h2>
